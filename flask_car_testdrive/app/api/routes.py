...
# надо прописать весь путь получения сертификата от начала и до конца с конфигами и композами.
# получаем тестовый сертификат с 1, потом меняем на 0, получаем реальный
# далее перенаправляем трафик с 80 http порта на 443 https через nginx
